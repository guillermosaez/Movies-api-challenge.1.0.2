...
public class CinemaContext : DbContext
{
    ...
    protected override void OnModelCreating(ModelBuilder modelBuilder)
    {
        ...
        {
            build.HasKey(entry => new { entry.TicketId, entry.SeatId });
            build.HasOne(entry => entry.Ticket).WithMany(entry => entry.Seats).HasForeignKey(entry => entry.TicketId);
            build.HasOne(entry => entry.Seat).WithMany(entry => entry.Tickets).HasForeignKey(entry => entry.SeatId);
        });
    }
}